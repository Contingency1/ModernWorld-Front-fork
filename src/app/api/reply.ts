...
export const REPLY = {
  ...
  async postReplies(commentNo: number, content: string) {
    try {
  ...
  async deleteReplies(commentNo: number, repliesNo: number) {
    try {
      const result: AxiosResponse = await instance.delete(
        `/${this.cm}/${commentNo}/${this.path}/${repliesNo}`,
      );
      alert('성공적으로 삭제되었습니다');
      return result;
    } catch (err) {
  ...
  async editReplies(commentNo: number, repliesNo: number, content: string) {
    try {
      if (confirm('수정하시겠습니까?')) {
